...
function strip_tags_smart(
    ...
    array $para_tags = array('td', 'th', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'form', 'title', 'pre')
)
{
//echo'<textarea style="width:100%;height:240px;">'.$allowable_tags.'</textarea>';
if (trim($allowable_tags) != ''){
...
foreach ($allowable_tags as $hgf){
	$hgf =  trim($hgf,'<> /');
	$hgf =  preg_replace('#-(.*)-#is','<\\1>',$hgf);
	$alle_tags[]= $hgf;
	$pair_no  = trim($hgf,'<> /');
	if (in_array ($pair_no,$pair_tags)) unset($pair_tags[array_search($pair_no, $pair_tags)]);
}
}
$allowable_tags =$alle_tags;
//var_export($allowable_tags);
    //return strip_tags($s);
    if (!is_array($s)){
preg_match_all ("#\[code\](.*)\[\/code\]#is",$s,$code_story); 
foreach ($code_story[0] as $key =>$value){$no_code['code_'.$key] = $value;}

preg_match_all ("#<code>(.*)</code>#is",$s,$code_story); 
foreach ($code_story[0] as $key =>$value){$no_codev['codev_'.$key] = $value;}

if (count($no_code))$s=strtr ($s,array_flip($no_code));
if (count($no_codev))$s=strtr ($s,array_flip($no_codev));
}
	...
	$s = str_replace (']]>', '', $s);
if (count($no_code))$s=strtr ($s,$no_code);
if (count($no_codev))$s=strtr ($s,$no_codev);
    return $s;
}
